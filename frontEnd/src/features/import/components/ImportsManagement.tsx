...
export const ImportsManagement: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const [importsData, peopleData] = await Promise.all([
        getImports(),
        getPeople(),
      ]);
  ...
  const deleteSelected = async () => {
    try {
      await Promise.all(selectedRows.map((id) => deleteImport(id)));
      setSelectedRows([]);
      await fetchData();
    } catch (err) {
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between p-5 border-b border-slate-50 gap-4">
          ...
          <div className="flex flex-wrap items-center justify-between lg:justify-end gap-4 border-t lg:border-t-0 pt-4 lg:pt-0 border-slate-50">
            <span className="text-[11px] font-medium text-slate-400 select-none">
              صف مرشح: {filteredImports.length}
            </span>
              ...
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  <th className="p-4 w-12 text-center select-none">
                    <input
                      type="checkbox"
                      className="w-4 h-4 rounded border-slate-200 text-emerald-600 focus:ring-0 cursor-pointer accent-emerald-600"
                      onChange={toggleAll}
                      checked={
                        filteredImports.length > 0 &&
                        filteredImports.every((e) =>
