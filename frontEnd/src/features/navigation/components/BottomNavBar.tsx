      ...
      {NAVIGATION_ITEMS.map((item) => {
        ...
        return (
          ...
          >
            ...
            <Icon
              size={18}
              className={`transition-colors duration-200 ${isActive ? THEME.primary.text : 'text-slate-400 hover:text-slate-600'
                }`}
              strokeWidth={isActive ? 2.2 : 1.8}
            />

            <span
              className={`text-[10px] font-semibold mt-1 transition-colors duration-200 ${isActive ? THEME.primary.text : 'text-slate-500'
                }`}
            >
              {item.label}
            </span>
